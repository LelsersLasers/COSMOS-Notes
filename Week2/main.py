        ...
        if currentState == FSM.START:
            ...
            for color in ["GREEN"]:
                ...
                if len(cnts[0]) > 0:
                    ...
                    break
        elif currentState == FSM.GREEN:
            # cv2.destroyWindow(winname)
            lowerThreshold = np.array([COLOR_HUE_RANGES["GREEN"][0], SAT_RANGES[0], VAL_RANGES[0]])
            upperThreshold = np.array([COLOR_HUE_RANGES["GREEN"][1], SAT_RANGES[1], VAL_RANGES[1]])

            green_mask = cv2.inRange(image_hsv, lowerThreshold, upperThreshold)
